...
import { generateInviteCodes, deleteInviteCode, regenerateInviteCode, updateInviteMessageTemplate, toggleInviteCopiedStatus } from '@/app/actions/admin';
import { Loader2, Trash2, Copy, Check, RefreshCw, MessageSquarePlus, CheckCircle2, Circle } from 'lucide-react';
...
export default function InviteListClient({ initialInvites, initialTemplate }: { initialInvites: Guest[], initialTemplate: string }) {
  ...
  const handleCopy = async (code: string, currentIsCopied: boolean | undefined) => {
    ...
    // Automatically mark as copied
    if (!currentIsCopied) {
      await handleToggleCopied(code, true);
    }
  };

  const handleToggleCopied = async (code: string, isCopied: boolean) => {
    setIsProcessing(true);
    const res = await toggleInviteCopiedStatus(code, isCopied);
    setIsProcessing(false);
    if (res.success) {
      router.refresh();
    } else {
      ...
      <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors duration-200">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          <button
            ...
            className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-700 dark:text-zinc-300 px-4 py-2 rounded-xl text-sm font-semibold hover:bg-gray-50 dark:hover:bg-zinc-800 shadow-sm flex items-center justify-center transition-all w-full sm:w-auto"
          >
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <input
              ...
              className="w-full sm:w-24 px-4 py-2 border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 text-gray-900 dark:text-zinc-100 rounded-xl text-sm focus:ring-2 focus:ring-gray-900 dark:focus:ring-zinc-100 outline-none transition-all"
            />
            <button
              onClick={handleGenerate}
              disabled={generating}
              className="bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-5 py-2 rounded-xl text-sm font-semibold hover:bg-gray-800 dark:hover:bg-white shadow-md disabled:opacity-70 flex items-center justify-center transition-all sm:flex-none shrink-0 whitespace-nowrap"
            >
              ...
              {currentInvites.map((invite, index) => (
                <tr key={invite.inviteCode} className="hover:bg-gray-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="px-5 py-4 font-mono font-medium text-gray-900 dark:text-zinc-100 flex items-center gap-2 whitespace-nowrap">
                    {invite.inviteCode}
                    <button onClick={() => handleCopy(invite.inviteCode, invite.isCopied)} className="text-gray-400 hover:text-gray-900 dark:hover:text-zinc-100 transition-colors" title="Copy Code">
                      {copiedCode === invite.inviteCode ? <Check className="w-3.5 h-3.5 text-green-600 dark:text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                    </button>
                    {invite.isCopied ? (
                      <button onClick={() => handleToggleCopied(invite.inviteCode, false)} className="text-green-600 dark:text-green-500 hover:text-green-700 dark:hover:text-green-400 transition-colors ml-1" title="Marked as copied (Click to unmark)">
                        <CheckCircle2 className="w-4 h-4" />
                      </button>
                    ) : (
                      <button onClick={() => handleToggleCopied(invite.inviteCode, true)} className="text-gray-300 dark:text-zinc-600 hover:text-gray-500 dark:hover:text-zinc-400 transition-colors ml-1" title="Unmarked (Click to mark as copied)">
